...
impl Encoder for NetworkGraphDTO {
    fn encode(&self) -> Vec<u8> {
        ...
        binary_writer.set_field_name("graph_nodes");
        binary_writer.step_in(ion_rs::IonType::List).expect("Error while entering an ion list");
        for graph_node in &self.graph_nodes {
            binary_writer.step_in(ion_rs::IonType::Struct).expect("Error while entering an ion struct");
            
            binary_writer.set_field_name("address");
            binary_writer.write_string(graph_node.get_address()).unwrap();
            
            binary_writer.step_out().unwrap();
        }
        binary_writer.step_out().unwrap();

        binary_writer.set_field_name("graph_edges");
        binary_writer.step_in(ion_rs::IonType::List).expect("Error while entering an ion list");
        for graph_edge in &self.graph_edges {
            binary_writer.step_in(ion_rs::IonType::Struct).expect("Error while entering an ion struct");
            
            binary_writer.set_field_name("src_addr");
            binary_writer.write_string(graph_edge.get_src_addr()).unwrap();
    
            binary_writer.set_field_name("dst_addr");
            binary_writer.write_string(graph_edge.get_dst_addr()).unwrap();
