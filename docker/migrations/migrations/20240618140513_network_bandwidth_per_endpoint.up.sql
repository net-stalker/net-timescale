...
CREATE INDEX IF NOT EXISTS idx_network_bandwidth_per_endpoint_tenant_network ON Network_Bandwidth_Per_Endpoint_Materialized_View USING HASH (Tenant_ID, Network_ID);

-- Create BRIN index on Frametime for space efficiency
CREATE INDEX IF NOT EXISTS idx_network_bandwidth_per_endpoint_frametime_brin ON Network_Bandwidth_Per_Endpoint_Materialized_View USING BRIN (Frametime);

-- Create separate indexes on Src_IP and Dst_IP
CREATE INDEX IF NOT EXISTS idx_network_bandwidth_per_endpoint_src_dst_ip ON Network_Bandwidth_Per_Endpoint_Materialized_View USING HASH (Src_IP, Dst_IP);

-- Create index on Packet_Length
CREATE INDEX IF NOT EXISTS idx_network_bandwidth_per_endpoint_packet_length ON Network_Bandwidth_Per_Endpoint_Materialized_View (Packet_Length);

-- Create index on Protocols for string operations
CREATE INDEX IF NOT EXISTS idx_network_bandwidth_per_endpoint_protocols ON Network_Bandwidth_Per_Endpoint_Materialized_View USING GIN (string_to_array(Protocols, ':'));
