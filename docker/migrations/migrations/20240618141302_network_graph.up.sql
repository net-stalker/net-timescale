...
CREATE INDEX IF NOT EXISTS idx_network_graph_tenant_network ON Network_Graph_Materialized_View USING HASH (Tenant_ID, Network_ID);

-- Create BRIN index on Frametime for space efficiency
CREATE INDEX IF NOT EXISTS idx_network_graph_frametime_brin ON Network_Graph_Materialized_View USING BRIN (Frametime);

-- Create separate indexes on Src_IP and Dst_IP
CREATE INDEX IF NOT EXISTS idx_network_graph_src_dst_ip ON Network_Graph_Materialized_View (Src_IP, Dst_IP);

-- Create index on Packet_Length
CREATE INDEX IF NOT EXISTS idx_network_graph_packet_length ON Network_Graph_Materialized_View (Packet_Length);

-- Create index on Protocols for string operations
CREATE INDEX IF NOT EXISTS idx_network_graph_protocols ON Network_Graph_Materialized_View USING GIN (string_to_array(Protocols, ':'));
