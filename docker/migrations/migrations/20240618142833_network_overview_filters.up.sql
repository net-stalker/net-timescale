...
CREATE MATERIALIZED VIEW IF NOT EXISTS Network_Overview_Materialized_View
AS
SELECT
...
CREATE INDEX IF NOT EXISTS idx_network_overview_src_ip ON Network_Overview_Materialized_View USING HASH (Src_IP);
CREATE INDEX IF NOT EXISTS idx_network_overview_dst_ip ON Network_Overview_Materialized_View USING HASH (Dst_IP);

-- Create index on Packet_Length
CREATE INDEX IF NOT EXISTS idx_network_overview_packet_length ON Network_Overview_Materialized_View (Packet_Length);

-- Create index on Protocols for string operations
CREATE INDEX IF NOT EXISTS idx_network_overview_protocols ON Network_Overview_Materialized_View USING GIN (string_to_array(Protocols, ':'));
